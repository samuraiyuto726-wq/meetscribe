...
def render_upload_tab():
      ...
      uploaded_file = st.file_uploader("Choose an audio file", type=["mp3", "wav", "m4a", "mp4", "webm", "ogg", "flac"], key="audio_upload")
                ...
                if st.button("Transcribe and Summarize", type="primary", use_container_width=True):
                              ...
                              with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
                                  ...
                                  st.success("Transcription complete!")
                                                                            ...
                                                                            st.markdown(summary)
                                                                            save_meeting(title=title, audio_path=tmp_path, transcript=transcript, summary=summary)
...
def render_record_tab():
    ...
    st.info("To use live recording, install streamlit-webrtc. For now, use the Upload tab to upload a pre-recorded meeting file.")
    st.write("Tip: Record your Zoom or Google Meet call using the built-in recording feature, then upload the file here.")
